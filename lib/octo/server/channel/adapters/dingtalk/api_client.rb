        ...
        class ApiClient
          ...
          def send_via_webhook(webhook_url, text, msg_type: :text)
            ...
            if resp.code.to_i != 200 || (data["errcode"] && data["errcode"] != 0)
              Octo::Logger.warn("[dingtalk] webhook send rejected (#{resp.code}): #{resp.body}")
            end
            data
          rescue => e
            Octo::Logger.warn("[dingtalk] webhook send failed: #{e.message}")
            {}
          end
          ...
          def download_message_file(download_code, robot_code, prefer_name: nil)
            ...
            Octo::Logger.warn("[dingtalk] download_message_file failed: #{e.message}")
            nil
          end
          ...
          private def fetch_download_url(download_code, robot_code)
            ...
            unless resp.code.to_i == 200 && data["downloadUrl"]
              Octo::Logger.warn("[dingtalk] fetch_download_url rejected (#{resp.code}): #{resp.body}")
              return nil
            end
            data["downloadUrl"]
          end
          ...
          private def download_to_disk(url, prefer_name: nil)
            ...
            saved    = Octo::Utils::FileProcessor.save(body: resp.body, filename: filename)
          ...
          def upload_media(path, kind:)
            ...
            unless resp.code.to_i == 200 && data["errcode"].to_i.zero? && data["media_id"]
              Octo::Logger.warn("[dingtalk] upload_media rejected (#{resp.code}): #{resp.body}")
              return nil
            end
            ...
            Octo::Logger.info("[dingtalk] upload_media ok type=#{type_str} media_id_len=#{mid.length} media_id_prefix=#{mid[0, 4].inspect}")
            mid
          rescue => e
            Octo::Logger.warn("[dingtalk] upload_media failed: #{e.message}")
            nil
          end
          ...
          def send_media(robot_code:, conv_type:, conv_id:, user_id:, media_id:, kind:, file_name: nil)
            ...
            if resp.code.to_i != 200
              Octo::Logger.warn("[dingtalk] send_media rejected (#{resp.code}): #{resp.body}")
            ...
            Octo::Logger.info("[dingtalk] send_media ok kind=#{kind} msgKey=#{msg_key}")
            { ok: true, data: data }
          rescue => e
            Octo::Logger.warn("[dingtalk] send_media failed: #{e.message}")
            { ok: false, error: e.message }
          end
