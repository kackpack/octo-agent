      ...
      module DingTalk
        class Adapter < Base
          ...
          def send_text(chat_id, text, reply_to: nil)
            ...
            unless webhook_url
              Octo::Logger.warn("[dingtalk] no valid sessionWebhook for chat #{chat_id} (expired or never received)")
          ...
          def send_file(chat_id, path, name: nil, reply_to: nil)
            unless File.exist?(path)
              Octo::Logger.warn("[dingtalk] send_file: file not found #{path}")
              return { ok: false, error: "file_not_found" }
            end

            route = resolve_route(chat_id)
            unless route
              Octo::Logger.warn("[dingtalk] send_file: no routing info for chat #{chat_id}")
            ...
            if kind == :file && !supported_file?(path)
              ...
              Octo::Logger.info("[dingtalk] send_file: unsupported extension .#{ext} (#{display_name})")
            ...
            unless media_id
              Octo::Logger.warn("[dingtalk] send_file: upload failed for #{path}")
          ...
          private def handle_frame(frame)
            ...
            data = begin
              ...
              Octo::Logger.warn("[dingtalk] failed to parse event data")
              return
            end
            ...
            Octo::Logger.info("[dingtalk] message from #{sender_id}: #{log_parts.join(' | ')}")
            @on_message&.call(event)
          rescue => e
            Octo::Logger.warn("[dingtalk] handle_frame error: #{e.message}")
          end
          ...
          private def extract_payload(data, robot_code)
            ...
            when "richText"
              ...
              Octo::Logger.info("[dingtalk] unsupported msgtype=#{msgtype}, ignoring")
            end
