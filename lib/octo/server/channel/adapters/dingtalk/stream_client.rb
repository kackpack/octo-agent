      ...
      module DingTalk
        ...
        class StreamClient
          ...
          def start(&on_event)
            @running  = true
            @on_event = on_event
            Octo::Logger.info("[dingtalk-ws] Starting stream client (client_id=#{@client_id})")

            while @running
              begin
                connect_and_listen
              rescue => e
                Octo::Logger.warn("[dingtalk-ws] Connection error: #{e.class}: #{e.message}")
          ...
          private def connect_and_listen
            Octo::Logger.info("[dingtalk-ws] Fetching stream endpoint...")
            endpoint, ticket = fetch_stream_endpoint
            full_url = "#{endpoint}?ticket=#{ticket}"
            Octo::Logger.info("[dingtalk-ws] Connecting to #{endpoint}")
            ...
            Octo::Logger.info("[dingtalk-ws] WebSocket connected")
            ...
            loop do
              ...
              unless ready
                Octo::Logger.warn("[dingtalk-ws] read timeout, reconnecting...")
                return
              end
              ...
              while (frame = @incoming.next)
                ...
                when :close
                  Octo::Logger.info("[dingtalk-ws] Server closed connection, will reconnect")
                  return
                end
              end
            end
          ...
          private def handle_frame(json_text)
            ...
            Octo::Logger.warn("[dingtalk-ws] JSON parse error: #{e.message}")
          end
          ...
          private def handle_system(frame)
            ...
            when "ping"
              ...
              Octo::Logger.info("[dingtalk-ws] pong sent")
            when "disconnect"
              Octo::Logger.warn("[dingtalk-ws] Server requested disconnect, will reconnect")
              @ws_socket&.close rescue nil
            end
          end
