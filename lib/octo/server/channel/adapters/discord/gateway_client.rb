      ...
      module Discord
        ...
        class GatewayClient
          ...
          def start(&on_event)
            ...
            while @running
              begin
                ...
                Octo::Logger.error("[DiscordGW] error: #{e.message}")
          ...
          private def connect_and_listen
            ...
            Octo::Logger.info("[DiscordGW] connecting to #{uri.host}:#{port} (resume=#{!@session_id.nil?})")
            ...
            loop do
              ...
              unless ready
                Octo::Logger.warn("[DiscordGW] read timeout, reconnecting")
                return
              end
            ...
            Octo::Logger.info("[DiscordGW] connection lost (#{e.class}: #{e.message})")
          ensure
            @ws_open = false
            @ws_socket = nil
            @heartbeat_thread&.kill
          ...
          private def handle_payload(payload)
            ...
            when 10
              ...
              Octo::Logger.info("[DiscordGW] hello, heartbeat_interval=#{@heartbeat_interval}ms")
            ...
            when 1
              send_heartbeat
            when 7
              Octo::Logger.info("[DiscordGW] server requested reconnect")
              @ws_socket&.close rescue nil
            when 9
              resumable = data == true
              Octo::Logger.warn("[DiscordGW] invalid session (resumable=#{resumable})")
              unless resumable
                @session_id = nil
                @resume_gateway_url = nil
                @last_seq = nil
          ...
          private def handle_dispatch(type, data)
            ...
            when "READY"
              ...
              user = data["user"] || {}
              Octo::Logger.info("[DiscordGW] READY as #{user["username"]} (id=#{user["id"]}), session=#{@session_id}")
            when "RESUMED"
              Octo::Logger.info("[DiscordGW] RESUMED session=#{@session_id}")
            when "MESSAGE_CREATE"
              @on_event&.call(type: :message, data: data)
            end
          rescue => e
            Octo::Logger.error("[DiscordGW] dispatch handler error (#{type}): #{e.message}\n#{e.backtrace.first(3).join("\n")}")
          end
          ...
          private def handle_close_frame(data)
            ...
            Octo::Logger.warn("[DiscordGW] close frame code=#{code} reason=#{reason}")
          ...
          private def send_identify
            Octo::Logger.info("[DiscordGW] sending Identify (intents=#{INTENTS})")
          ...
          private def send_resume
            Octo::Logger.info("[DiscordGW] sending Resume (session=#{@session_id} seq=#{@last_seq})")
            send_payload(
              op: 6,
          ...
          private def send_heartbeat
            unless @heartbeat_acked
              Octo::Logger.warn("[DiscordGW] missed heartbeat ack, forcing reconnect")
              @ws_socket&.close rescue nil
              return
            end
            ...
            @heartbeat_thread = Thread.new do
              ...
              loop do
                ...
                begin
                  ...
                  Octo::Logger.warn("[DiscordGW] heartbeat send failed: #{e.message}")
                  @ws_socket&.close rescue nil
                  break
                end
                sleep interval_s
