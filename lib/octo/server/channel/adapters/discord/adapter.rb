        ...
        class Adapter < Base
          ...
          def start(&on_message)
            ...
            begin
              me = @api.me
              @bot_user_id = me["id"]
              Octo::Logger.info("[DiscordAdapter] authenticated as #{me["username"]} (id=#{@bot_user_id})")
            rescue ApiClient::ApiError => e
              Octo::Logger.error("[DiscordAdapter] /users/@me failed, not retrying: #{e.message}")
              return
            end
            ...
            Octo::Logger.error("[DiscordAdapter] Authentication failed, not retrying: #{e.message}")
          end
          ...
          def send_text(chat_id, text, reply_to: nil)
            ...
            Octo::Logger.error("[DiscordAdapter] send_text failed: #{e.message}")
            { message_id: nil }
          end
          ...
          def update_message(chat_id, message_id, text)
            ...
            Octo::Logger.warn("[DiscordAdapter] update_message failed: #{e.message}")
            false
          end
          ...
          def send_file(chat_id, path, name: nil)
            ...
            Octo::Logger.error("[DiscordAdapter] send_file failed: #{e.message}")
            nil
          end
          ...
          private def handle_message(msg)
            ...
            if chat_type == :group
              if @bot_user_id.nil?
                Octo::Logger.warn("[DiscordAdapter] bot_user_id unavailable; dropping group message")
                return
              end
            ...
            Octo::Logger.error("[DiscordAdapter] handle_message error: #{e.message}\n#{e.backtrace.first(3).join("\n")}")
            begin
          ...
          private def process_attachments(attachments, chat_id)
            ...
            attachments.each do |att|
              ...
              if mime && mime.start_with?("image/")
                ...
                files << Octo::Utils::FileProcessor.save(body: body, filename: filename)
              end
            end
            files
          rescue => e
            Octo::Logger.warn("[DiscordAdapter] process_attachments error: #{e.message}")
            files
          end
