...
module Octo
  module Server
    # Discover locally-running Octo server(s) by scanning PID files
    # written by Master at /tmp/octo-master-<port>.pid.
    #
    # Used by the CLI (bare `octo agent` mode) to auto-detect a sibling
    ...
    # OCTO_SERVER_HOST / OCTO_SERVER_PORT.
    #
    ...
    module Discover
      PID_FILE_GLOB = File.join(Dir.tmpdir, "octo-master-*.pid").freeze
      PID_FILE_REGEX = /octo-master-(\d+)\.pid\z/.freeze

      module_function

      # Find the first live Octo server on this machine.
      #
      ...
      # Find all live Octo servers on this machine.
      #
      # A PID file is considered "live" when:
      #   1. The filename matches octo-master-<port>.pid
