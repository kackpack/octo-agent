...
name: browser-setup
description: |
  Configure the browser tool for Octo. Guides the user through Chrome or Edge setup,
  verifies the connection, and writes ~/.octo/browser.yml.
...
Configure the browser tool for Octo. Config is stored at `~/.octo/browser.yml`.
...
```bash
bash ~/.octo/scripts/install_browser.sh
```
...
```bash
curl -s http://${OCTO_SERVER_HOST}:${OCTO_SERVER_PORT}/api/browser/status
```

This returns JSON with `daemon_running` and `enabled` status. **Ignore the result for now** — we just need to see if the Octo server is running.
...
```bash
curl -s -X POST http://${OCTO_SERVER_HOST}:${OCTO_SERVER_PORT}/api/browser/configure \
...
> ⚠️ Failed to save configuration via API. You may need to run `/browser-setup` again after restarting Octo.
...
```bash
test -f ~/.octo/browser.yml && cat ~/.octo/browser.yml
```
...
```bash
curl -s -f http://${OCTO_SERVER_HOST}:${OCTO_SERVER_PORT}/api/browser/status
```
...
Configuration
  [✅] Config file found (~/.octo/browser.yml)
