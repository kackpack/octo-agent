...
import yaml  # PyYAML — available in most Python envs; fallback parser below
...
def _load_octo_api_config() -> dict:
    ...
    # Try PyYAML first, then fall back to minimal regex-based parser
    try:
...
def _parse_yaml_fallback(raw: str) -> list:
    """Minimal regex-based YAML list parser as fallback when PyYAML is unavailable."""
    entries = []
    current: dict = {}
...
def _call_llm(prompt: str, config: dict, timeout: int = 300) -> str:
    """Call the LLM API (OpenAI-compatible) with the given prompt.
    ...
    if not choices:
        raise RuntimeError(f"LLM API returned no choices. Response: {body}")
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # If model produced a description over the hard limit, ask it to shorten
    if len(description) > 1024:
        ...
        shorten_text = _call_llm(shorten_prompt, config)
