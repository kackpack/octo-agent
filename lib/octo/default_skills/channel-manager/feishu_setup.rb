...
#
# Strategy: Use browser (via octo server's /api/tool/browser) ONLY to:
...
#
# Environment (injected by octo server):
#   OCTO_SERVER_PORT — port the octo server is listening on
#   OCTO_SERVER_HOST — host the octo server is listening on
#   OCTO_PRODUCT_NAME — product name (default: Octo)
...
PRODUCT_NAME      = ENV.fetch("OCTO_PRODUCT_NAME", "Octo")
...
# ---------------------------------------------------------------------------
# ToolClient — proxies browser calls through /api/tool/browser on octo server
# ---------------------------------------------------------------------------
...
def run_setup(browser, api)
  ...
  # ── Phase 4: Write credentials to octo server and wait for WS ─────────
  step "Phase 4 — Writing credentials to octo server..."

  # Helper: one-shot HTTP request to octo server (new connection each time, no keep-alive issues)
  ...
  begin
    ...
    warn "Could not reach octo server (#{e.message}) — continuing..."
  end
...
tool_client = ToolClient.new(OCTO_SERVER_URL)
