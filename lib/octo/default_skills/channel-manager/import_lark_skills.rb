...
#   under ~/.agents/skills/lark-*, which octo's SkillLoader does NOT
#   scan. This importer copies them into ~/.octo/skills/lark-imports/ so
...
#
# Usage:
#   importer = Octo::ChannelSetup::LarkSkillsImporter.new
#   result = importer.run
...
module Octo
  module ChannelSetup
    class LarkSkillsImporter
      ...
      DEFAULT_TARGET_DIR = File.join(Dir.home, '.octo', 'skills', 'lark-imports')
      SKILL_PREFIX       = 'lark-'

      # @param source_dir [String] directory containing lark-cli installed skills
      # @param target_dir [String] destination under ~/.octo/skills/
...
if $PROGRAM_NAME == __FILE__
  result = Octo::ChannelSetup::LarkSkillsImporter.new.run
