...
module Octo
  ...
  # Octo platform API (www.octo.com and its fallback domain).
  #
  # Features:
  ...
  #   - Override via OCTO_LICENSE_SERVER env var (auto-detected, used in development)
  #
  # Usage:
  #   client = Octo::PlatformHttpClient.new
  #   result = client.post("/api/v1/licenses/activate", payload)
  #   # result => { success: true, data: {...} }
  #   #        or { success: false, error: "...", data: {} }
  class PlatformHttpClient
    # Primary CDN-accelerated endpoint
    PRIMARY_HOST  = "https://www.octo.com"
    ...
    #   - When OCTO_LICENSE_SERVER is set → single host (dev override, no failover)
    #   - Otherwise                   → [PRIMARY_HOST, FALLBACK_HOST]
    def initialize
      if (override = ENV["OCTO_LICENSE_SERVER"]) && !override.empty?
        @hosts = [override]
      else
