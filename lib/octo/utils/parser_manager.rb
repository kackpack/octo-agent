...
module Octo
  module Utils
    # Manages user-space parsers in ~/.octo/parsers/.
    #
    # On first use, default parser scripts are copied from the gem's
    # default_parsers/ directory into ~/.octo/parsers/. After that,
    ...
    module ParserManager
      PARSERS_DIR         = File.expand_path("~/.octo/parsers").freeze
      ...
      # Ensure ~/.octo/parsers/ exists and all default parsers are present.
      ...
      #
      #   On startup, per-file:
      #     - If the file does NOT exist in ~/.octo/parsers/ → copy it.
      #     - If it exists:
      ...
      def self.parse(file_path)
        ...
        stdout = Octo::Utils::Encoding.to_utf8(raw_stdout)
        stderr = Octo::Utils::Encoding.to_utf8(raw_stderr)
