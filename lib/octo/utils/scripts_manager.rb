...
module Octo
  module Utils
    # Manages user-space shell scripts in ~/.octo/scripts/.
    #
    # On first use, bundled scripts are copied from the gem's scripts/
    # directory into ~/.octo/scripts/. The user-space copy is always
    ...
    # via gem version stamp in ~/.octo/scripts/.version).
    module ScriptsManager
      SCRIPTS_DIR         = File.expand_path("~/.octo/scripts").freeze
      ...
      # Copy bundled scripts to ~/.octo/scripts/ if missing or outdated.
      # Called once at agent startup — fast (no-op after first run).
      def self.setup!
        FileUtils.mkdir_p(SCRIPTS_DIR)

        current_version = Octo::VERSION
