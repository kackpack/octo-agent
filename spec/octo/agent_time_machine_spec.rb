...
RSpec.describe "Octo::Agent TimeMachine" do
  let(:client) do
  ...
  let(:working_dir) { Dir.mktmpdir("octo_time_machine_test") }
  let(:agent) { Octo::Agent.new(client, config, working_dir: working_dir, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual) }

  # Helper to get snapshot directory for a task
  def snapshot_dir(task_id)
    File.join(Dir.home, ".octo", "snapshots", agent.session_id, "task-#{task_id}")
  end
  ...
  describe "#active_messages" do
    ...
    it "includes system messages without task_id" do
      ...
      system_msg = all.pop
      agent.instance_variable_set(:@history, Octo::MessageHistory.new([system_msg] + all))
      agent.instance_variable_set(:@active_task_id, 1)
  ...
  describe "integration with session serialization" do
    ...
    it "restores time machine state from session" do
      ...
      new_agent = Octo::Agent.new(client, config, working_dir: working_dir, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual)
