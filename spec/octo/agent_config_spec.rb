...
RSpec.describe Octo::AgentConfig do
  ...
  describe "OctoEnv environment variables" do
    describe "default model" do
      it "loads from OCTO_XXX env vars when config is empty" do
        with_env(
          "OCTO_API_KEY" => "sk-octo-test",
          "OCTO_BASE_URL" => "https://api.octo.test",
          "OCTO_MODEL" => "claude-test-model",
          "OCTO_ANTHROPIC_FORMAT" => "false"
        ) do
          with_temp_config do |config_file|
            FileUtils.rm_f(config_file)
            ...
            expect(config.models.first["api_key"]).to eq("sk-octo-test")
            expect(config.models.first["base_url"]).to eq("https://api.octo.test")
      ...
      it "uses default model name if OCTO_MODEL not set" do
        with_env("OCTO_API_KEY" => "sk-test") do
          with_temp_config do |config_file|
            FileUtils.rm_f(config_file)
    ...
    describe "lite model" do
      it "loads from OCTO_LITE_XXX env vars" do
        with_env(
          "OCTO_API_KEY" => "sk-default",
          "OCTO_LITE_API_KEY" => "sk-lite",
          "OCTO_LITE_MODEL" => "claude-haiku-test"
        ) do
          with_temp_config do |config_file|
            FileUtils.rm_f(config_file)
    ...
    describe "priority: config file > OCTO_XXX > ClaudeCode" do
      ...
      it "prefers OCTO_XXX over ClaudeCode env vars" do
        with_env(
          "OCTO_API_KEY" => "sk-octo",
          "ANTHROPIC_API_KEY" => "sk-claude"
        ) do
          with_temp_config do |config_file|
            FileUtils.rm_f(config_file)
            ...
            expect(config.models.first["api_key"]).to eq("sk-octo")
          end
        end
      end

      # it "falls back to ClaudeCode if OCTO_XXX not set" do
  ...
  describe "Octo::Providers.find_by_base_url" do
    ...
    it "returns nil for unknown base URLs" do
      expect(Octo::Providers.find_by_base_url("https://unknown.example.com")).to be_nil
    end

    it "returns nil for nil input" do
      expect(Octo::Providers.find_by_base_url(nil)).to be_nil
    end
  end
  ...
  describe "Octo::Providers.lite_model" do
    ...
    it "returns nil for providers without any lite mapping (e.g. minimax)" do
      expect(Octo::Providers.lite_model("minimax")).to be_nil
      expect(Octo::Providers.lite_model("minimax", "MiniMax-M2")).to be_nil
    end

    it "returns nil for unknown provider IDs" do
      expect(Octo::Providers.lite_model("nonexistent")).to be_nil
      expect(Octo::Providers.lite_model("nonexistent", "anything")).to be_nil
    end
  end
  ...
  describe "#default_working_dir" do
    it "returns nil by default (no config, no env)" do
      with_env("OCTO_WORKSPACE_DIR" => nil) do
        config = described_class.new
        expect(config.default_working_dir).to be_nil
      end
    end

    it "reads from OCTO_WORKSPACE_DIR env var" do
      with_env("OCTO_WORKSPACE_DIR" => "/tmp/custom-workspace") do
        config = described_class.new
