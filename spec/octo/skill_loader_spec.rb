...
RSpec.describe Octo::SkillLoader do
  ...
  describe "#initialize" do
    it "initializes with working directory" do
      loader = described_class.new(working_dir: working_dir)
    ...
    it "uses current directory when no working_dir given" do
      ...
      loader = described_class.new(working_dir: nil)
      expect(loader).to be_a(described_class)
    ensure
  ...
  describe "#load_all" do
    context "with no skills directories" do
      it "returns default skills" do
        ...
        # User may have global skills in ~/.octo/skills/
    ...
    context "with skills in project .octo/skills/" do
      it "loads skills from .octo/skills/" do
        # Create skill in .octo/skills/
        skills_dir = File.join(working_dir, ".octo", "skills")
      ...
      it "does NOT load project skills when working_dir is nil" do
        # Create a separate temp directory to avoid touching any real .octo/skills/
        test_temp_dir = Dir.mktmpdir("skill-loader-test")
        
        begin
          skills_dir = File.join(test_temp_dir, ".octo", "skills")
          FileUtils.mkdir_p(skills_dir)
  ...
  describe "#create_skill" do
    context "with project location" do
      it "creates skill in project .octo/skills/" do
        loader = described_class.new(working_dir: working_dir)
        ...
        project_skills_dir = File.join(working_dir, ".octo", "skills")
    ...
    it "validates skill name format" do
      ...
      end.to raise_error(Octo::AgentError, /Invalid skill name/)
    end
  end

  describe "#toggle_skill" do
    let(:loader) { described_class.new(working_dir: working_dir) }
    ...
    let(:skill_file) do
      File.join(working_dir, ".octo", "skills", "my-skill", "SKILL.md")
    end
    ...
    it "raises error for system skills" do
      ...
      end.to raise_error(Octo::AgentError, /Cannot toggle system skill/)
    end

    it "raises error for unknown skill" do
      expect do
        loader.toggle_skill("nonexistent", enabled: true)
      end.to raise_error(Octo::AgentError, /Skill not found/)
    end
  end
  ...
  describe "#delete_skill" do
    it "deletes an existing skill" do
      ...
      skill_dir = File.join(working_dir, ".octo", "skills", "to-delete")
  ...
  describe "brand skills removed in octo fork" do
    it "no longer loads brand skills" do
      loader = described_class.new(working_dir: working_dir)
      loader.load_all

      expect(loader.all_skills.map(&:identifier)).not_to include("free-demo")
    end
  end
end
