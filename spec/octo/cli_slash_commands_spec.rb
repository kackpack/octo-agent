...
RSpec.describe Octo::CLI, "UI2 slash commands" do
  let(:cli) { Octo::CLI.new }
  let(:working_dir) { Dir.pwd }
  let(:agent_config) { Octo::AgentConfig.new }
  ...
  let(:agent_profile) { instance_double(Octo::AgentProfile, name: "coding") }
  let(:skill_loader) { instance_double(Octo::SkillLoader) }

  let(:agent) do
    instance_double(Octo::Agent,
  ...
  before do
    ...
    allow(Octo::UI2::TerminalDetector).to receive(:detect_dark_background).and_return(true)
    allow(Octo::UI2::ThemeManager.instance).to receive(:set_background_mode)
    allow(Octo::UI2::ThemeManager).to receive(:available_themes).and_return(%i[hacker minimal])

    # Return our fake UIController instead of building a real one
    allow(Octo::UI2::UIController).to receive(:new).and_return(ui_controller)
  ...
  describe "/clear" do
    ...
    before do
      ...
      allow(Octo::SessionManager).to receive(:generate_id).and_return("fresh-session-id")
      allow(Octo::Agent).to receive(:new).and_return(new_agent)
      allow(new_agent).to receive(:instance_variable_set)
    end

    it "creates a new Agent with a fresh session_id" do
      expect(Octo::Agent).to receive(:new).with(
        client, agent_config,
