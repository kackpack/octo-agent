...
RSpec.describe "CLI client staleness regression (DSK → Opus → /clear)" do
  let(:config_path) { Dir.mktmpdir("octo-cli-regression") }
  let(:agent_config) do
    Octo::AgentConfig.new.tap do |cfg|
      cfg.instance_variable_set(:@models, [
        {
          "id"       => "m-dsk",
          "name"     => "DSK",
          "model"    => "dsk-chat",
          "api_key"  => "octo-dsk-key",
  ...
  it "after /config switch THEN /clear (new Agent from factory), the fresh client still has @use_bedrock=true" do
    ...
    #   agent = Octo::Agent.new(client_factory.call, agent_config, ...)
    fresh_agent = Octo::Agent.new(
      client_factory.call,
      agent_config,
      working_dir:  Dir.pwd,
      ui:           nil,
