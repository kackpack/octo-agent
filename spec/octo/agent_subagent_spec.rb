...
RSpec.describe Octo::Agent, "#fork_subagent" do
  let(:config) do
  ...
  before do
    ...
    allow(Octo::Client).to receive(:new).and_return(client)
  ...
  describe "#fork_subagent" do
    it "creates a subagent with the same messages" do
      ...
      expect(subagent).to be_a(Octo::Agent)
    ...
    it "deep clones messages to avoid cross-contamination" do
      ...
      agent.instance_variable_set(:@history, Octo::MessageHistory.new(original_messages))
    ...
    it "marks subagent with metadata" do
      agent.instance_variable_set(:@history, Octo::MessageHistory.new([{ role: "user", content: "test" }]))
    ...
    context "with model switching" do
      ...
      it "raises error for non-existent model" do
        ...
        }.to raise_error(Octo::AgentError, /not found in config/)
      end
    end
    ...
    context "with model: 'lite' (virtual overlay)" do
      ...
      it "falls back to the current model when no lite is resolvable" do
        # Use a self-hosted base_url AND a non-octo api_key: no provider
