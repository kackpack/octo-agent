...
RSpec.describe Octo::PlatformHttpClient, "#download_file" do
  ...
  describe "primary → fallback failover" do
    ...
    it "retries the primary host twice, then swaps to fallback host" do
      err = Octo::PlatformHttpClient::RetryableNetworkError.new("Timeout")
    ...
    it "reports a structured failure when every host is exhausted" do
      err = Octo::PlatformHttpClient::RetryableNetworkError.new("Connection error: reset")
    ...
    it "does NOT swap host for non-primary URLs (e.g. S3 presigned URLs)" do
      external = "https://octo-skills.s3.amazonaws.com/abc.zip?sig=xyz"
      err = Octo::PlatformHttpClient::RetryableNetworkError.new("Timeout")
  ...
  describe "URL host rewriting" do
    it "preserves path + query when swapping to the fallback host" do
      ...
      calls = stub_stream([
        Octo::PlatformHttpClient::RetryableNetworkError.new("Timeout"),
        Octo::PlatformHttpClient::RetryableNetworkError.new("Timeout"),
        :ok
      ])
      allow(client).to receive(:sleep)
