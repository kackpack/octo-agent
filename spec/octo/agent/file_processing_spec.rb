...
RSpec.describe "Agent file processing" do
  ...
  let(:config) do
    Octo::AgentConfig.new(model: "gpt-4o", permission_mode: :auto_approve)
  end
  let(:agent) do
    Octo::Agent.new(client, config,
  ...
  describe "disk file parsing is called during agent.run" do
    it "calls FileProcessor.process_path for each non-image disk file" do
      Dir.mktmpdir do |dir|
        ...
        ref = Octo::Utils::FileProcessor::FileRef.new(
          name: "report.pdf", type: :pdf,
          original_path: path, preview_path: "#{path}.preview.md"
        )
        allow(Octo::Utils::FileProcessor).to receive(:process_path)
        ...
        expect(Octo::Utils::FileProcessor).to have_received(:process_path)
          .with(path, name: "report.pdf")
      end
    end
  ...
  describe "file_prompt injected into history" do
    it "includes preview path when parse succeeds" do
      Dir.mktmpdir do |dir|
        ...
        allow(Octo::Utils::FileProcessor).to receive(:process_path).and_return(ref)
    ...
    it "includes parse_error repair hint when parse fails" do
      Dir.mktmpdir do |dir|
        ...
        parser_path = "/home/.octo/parsers/pdf_parser.rb"
  ...
  describe "provider vision capability gating" do
    ...
    it "keeps images inline (vision_images path) for a vision-capable provider" do
      # octo + Claude → vision:true. process_path must NOT be called
    ...
    it "downgrades octo+DeepSeek images via the model-level override" do
