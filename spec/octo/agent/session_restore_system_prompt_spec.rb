...
RSpec.describe "Agent#restore_session refreshes system prompt" do
  ...
  let(:config) do
    Octo::AgentConfig.new(model: "gpt-3.5-turbo", permission_mode: :auto_approve)
  end
  ...
  it "replaces stale system message with a freshly built system prompt on restore" do
    Dir.mktmpdir do |tmpdir|
      ...
      skill_dir = File.join(tmpdir, ".octo", "skills", "new-skill")
      ...
      allow_any_instance_of(Octo::SkillLoader).to receive(:load_global_octo_skills).and_return([])
      allow_any_instance_of(Octo::SkillLoader).to receive(:load_brand_skills).and_return([])
