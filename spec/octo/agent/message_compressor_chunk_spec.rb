...
RSpec.describe "Compression chunk MD archiving" do
  ...
  let(:agent_class) do
    Class.new do
      include Octo::Agent::MessageCompressorHelper
  ...
  before do
    stub_const("Octo::SessionManager::SESSIONS_DIR", sessions_dir)
  end
  ...
  describe "SessionManager cleanup" do
    let(:manager) { Octo::SessionManager.new(sessions_dir: sessions_dir) }
  ...
  describe Octo::MessageCompressor do
    describe "#rebuild_with_compression" do
      ...
      it "injects chunk anchor into compressed summary when chunk_path is provided" do
        chunk_path = "/home/user/.octo/sessions/2026-03-08-10-00-00-abc12345-chunk-1.md"
  ...
  describe "SessionManager chunk discovery" do
    let(:sm) { Octo::SessionManager.new(sessions_dir: sessions_dir) }
  ...
  describe "end-to-end: consecutive compressions produce unique chunk files" do
    ...
    let(:cfg) do
      instance_double(Octo::AgentConfig, enable_compression: true)
    end
    ...
    subject(:full_agent) do
      ...
      a.config = cfg
      a.history = Octo::MessageHistory.new([])
      a.message_compressor = Octo::MessageCompressor.new(nil)
      a
    end
    ...
    def run_compression_round(agent, round_number)
      ...
      if pre_existing.empty?
        agent.history = Octo::MessageHistory.new([system, *new_turns])
      else
      ...
      sm = Octo::SessionManager.new(sessions_dir: sessions_dir)
