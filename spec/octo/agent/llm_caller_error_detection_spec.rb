...
require "spec_helper"
# spec_helper already loads the full octo gem, which defines:
#   Octo::BadRequestError, Octo::RetryableError, Octo::AgentError
require "octo/agent/llm_caller"

# Unit tests for the helper detector methods inside Octo::Agent::LlmCaller.
#
...
RSpec.describe Octo::Agent::LlmCaller do
  let(:host_class) do
    Class.new do
      include Octo::Agent::LlmCaller
  ...
  describe "#context_too_long_error?" do
    ...
    PROVIDER_ERRORS.each do |label, raw_message|
      it "detects: #{label}" do
        err = Octo::BadRequestError.new(raw_message)
    ...
    context "with non-BadRequestError inputs" do
      ...
      it "returns false for a RetryableError even if the message would match" do
        err = Octo::RetryableError.new("context length exceeded")
        expect(host.context_too_long_error?(err)).to be false
      end
    ...
    context "robustness" do
      ...
      it "handles an empty error message safely" do
        expect(host.context_too_long_error?(Octo::BadRequestError.new(""))).to be false
      end
    end
  end
