...
RSpec.describe Octo::Agent do
  ...
  let(:agent) { described_class.new(client, config, working_dir: Dir.pwd, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual) }
  ...
  describe "message compression" do
    ...
    let(:compression_agent) { described_class.new(client, compression_config, working_dir: Dir.pwd, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual) }
    ...
    it "can be disabled via config" do
      no_compression_config = Octo::AgentConfig.new(
        permission_mode: :auto_approve,
        enable_compression: false,
        keep_recent_messages: 5
      )
      no_compression_agent = described_class.new(client, no_compression_config, working_dir: Dir.pwd, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual)
    ...
    context "when compression is interrupted by new user input" do
      ...
      it "restores @compression_level to its pre-compression value after interrupt" do
        ...
        allow(client).to receive(:send_messages_with_tools)
          .and_raise(Octo::AgentInterrupted, "New input received")

        # think() should propagate the interrupt (it only catches it in ensure)
        expect { compression_agent.send(:think) }.to raise_error(Octo::AgentInterrupted)
  ...
  describe ".from_session" do
    ...
    context "when session ended with error" do
      ...
      it "triggers session_rollback hook" do
        ...
        agent_with_hook = described_class.new(client, config, working_dir: Dir.pwd, ui: nil, profile: "coding", session_id: Octo::SessionManager.generate_id, source: :manual)
  ...
  describe "truncated response handling" do
    ...
    it "does not retry on unrelated 400 errors" do
      ...
      allow(client).to receive(:send_messages_with_tools) do |_messages, **_opts|
        call_count += 1
        raise Octo::BadRequestError, "[LLM] Client request error: invalid tool schema"
      end

      expect { agent.run("go") }.to raise_error(Octo::BadRequestError, /invalid tool schema/)
  ...
  describe "#inject_todo_reminder" do
    let(:todo_tool) { instance_double(Octo::Tools::TodoManager) }
  ...
  describe "inline skill injection via agent loop" do
    ...
    def build_agent_with_inline_skill(tmpdir)
      ...
      skill_dir = File.join(tmpdir, ".octo", "skills", "magic-skill")
  ...
  describe "dangling tool_calls cleanup (API error 2013 prevention)" do
    ...
    context "when previous task was interrupted (AgentInterrupted) after think() appended tool_calls" do
      ...
      it "drops dangling tool_calls so the next task does not trigger error 2013" do
        ...
        allow(client).to receive(:send_messages_with_tools)
          ...
          .once
        allow(agent).to receive(:act).and_raise(Octo::AgentInterrupted, "user pressed Ctrl+C")
        allow(agent).to receive(:inject_memory_prompt!).and_return(false)

        expect { agent.run("task one") }.to raise_error(Octo::AgentInterrupted)
    ...
    context "when previous task ended with AgentError after think() appended tool_calls" do
      ...
      it "API receives no dangling tool_calls when error occurred before observe()" do
        ...
        allow(agent).to receive(:act).and_raise(Octo::AgentError, "simulated API failure")
        allow(agent).to receive(:inject_memory_prompt!).and_return(false)

        expect { agent.run("task one") }.to raise_error(Octo::AgentError)
        expect(agent.history.pending_tool_calls?).to be true
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # Brand skills removed in octo fork
  describe "brand skill end-to-end" do
    it "does not support encrypted skill files" do
      Dir.mktmpdir do |tmp|
        skill_dir = File.join(tmp, ".octo", "skills", "plain-skill")
        FileUtils.mkdir_p(skill_dir)
        ...
        skill = plain_agent.instance_variable_get(:@skill_loader).find_by_name("plain-skill")
