...
#
# Background (root cause tracked in memory `octo-agent-silent-stop-bug`):
#
...
RSpec.describe Octo::Agent, "upstream tool-call truncation recovery" do
  let(:config) do
  ...
  describe "persistent truncation never recovers" do
    it "eventually raises AgentError after exhausting retries (no infinite loop)" do
      ...
      }.to raise_error(Octo::AgentError, /Service unavailable|Upstream truncated/i)
    end
  end
end
