...
require "octo/session_manager"
require "octo/agent_config"
require "octo/server/session_registry"

RSpec.describe Octo::Server::SessionRegistry do
  let(:default_config) { Octo::AgentConfig.new }
  ...
  describe "#snapshot" do
    it "returns a row with the same shape as #list for the given session" do
      Dir.mktmpdir("octo_snapshot_spec") do |dir|
        ...
        manager  = Octo::SessionManager.new(sessions_dir: dir)
  ...
  describe "#running_full?" do
    ...
    it "respects agent_config max_running_agents" do
      config = Octo::AgentConfig.new(max_running_agents: 2)
  ...
  describe "#evict_excess_idle!" do
    it "evicts oldest idle agents when exceeding default limit" do
      Dir.mktmpdir("octo_evict_spec") do |dir|
        manager  = Octo::SessionManager.new(sessions_dir: dir)
        registry = described_class.new(session_manager: manager, agent_config: default_config)
    ...
    it "respects agent_config max_idle_agents" do
      Dir.mktmpdir("octo_evict_spec") do |dir|
        config = Octo::AgentConfig.new(max_idle_agents: 2)
        manager  = Octo::SessionManager.new(sessions_dir: dir)
