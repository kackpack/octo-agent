...
require "octo/utils/trash_directory"
require "octo/tools/trash_manager"
...
module HttpServerTrashSpecHelpers
  ...
  def build_server
    ...
    Octo::Server::HttpServer.new(
      ...
      sessions_dir:   Dir.mktmpdir("octo_trash_spec_sessions")
    )
  end
  ...
  def seed_trash_file(project_root:, basename:, content: "bye", deleted_at: Time.now.utc.iso8601)
    td = Octo::TrashDirectory.new(project_root)
...
RSpec.describe Octo::Server::HttpServer, "creator trash endpoints" do
  include HttpServerTrashSpecHelpers

  # Redirect GLOBAL_TRASH_ROOT to a tmpdir for the duration of each spec so we
  # don't pollute the developer's real ~/.octo/trash.
  let!(:trash_root) { Dir.mktmpdir("octo_trash_root_spec") }
  before { stub_const("Octo::TrashDirectory::GLOBAL_TRASH_ROOT", trash_root) }
  ...
  describe "DELETE /api/trash (bulk empty mode)" do
    it "empties all files when days_old=0 and no project filter is given" do
      ...
      [project_a, project_b].each do |root|
        trash_dir = Octo::TrashDirectory.new(root).trash_dir
    ...
    it "only deletes files older than `days_old` days" do
      ...
      trash_dir = Octo::TrashDirectory.new(project_a).trash_dir
