...
require "tmpdir"
require "fileutils"
require "octo/server/http_server"
require "octo/agent_config"
...
module HttpServerSpecHelpers
  ...
  def with_server(agent_config:, client_factory: -> { double("client") }, sessions_dir: nil)
    dir = sessions_dir || Dir.mktmpdir("octo_http_spec_sessions")
    server = Octo::Server::HttpServer.new(
      host:           "127.0.0.1",
...
RSpec.describe Octo::Server::HttpServer do
  include HttpServerSpecHelpers

  let(:tmpdir) { Dir.mktmpdir("octo_http_server_spec") }
  ...
  let(:agent_config) do
    ...
    stub_const("Octo::AgentConfig::CONFIG_FILE", config_file)
    cfg
  end
  ...
  describe "GET /api/sessions" do
    ...
    describe "pinned sessions always appear on the first page" do
      ...
      it "includes an OLD pinned session in the first page even when limit is small" do
        ...
        Dir.mktmpdir("octo_pin_spec") do |dir|
  ...
  describe "POST /api/config/test" do
    it "returns ok: true when connection succeeds" do
      ...
      with_server(agent_config: agent_config, client_factory: client_factory) do |server|
        allow(Octo::Client).to receive(:new).and_return(test_client)
    ...
    it "uses stored key when masked placeholder is sent" do
      ...
      with_server(agent_config: agent_config) do |server|
        expect(Octo::Client).to receive(:new) do |key, **|
  ...
  describe "GET /api/sessions/:id/skills" do
    ...
    it "returns profile-filtered user_invocable skills for a session" do
      with_server(agent_config: agent_config) do |server|
        ...
        mock_skill = instance_double(Octo::Skill,
        ...
        mock_loader = instance_double(Octo::SkillLoader,
          load_all:              nil,
  ...
  describe "WebSocket subscribe replays inbox state" do
    let(:sessions_dir) { Dir.mktmpdir("octo_ws_sub_spec") }
  ...
  describe "interrupt re-broadcasts inbox queue status" do
    let(:sessions_dir) { Dir.mktmpdir("octo_interrupt_inbox_spec") }
  ...
  describe "#interrupt_all_agents (private)" do
    let(:sessions_dir) { Dir.mktmpdir("octo_interrupt_spec_sessions") }
    ...
    # Stand-in for Octo::Agent. We only need the surface that
