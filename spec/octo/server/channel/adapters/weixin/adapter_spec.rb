...
require "octo/server/channel/adapters/weixin/api_client"
require "tempfile"

RSpec.describe Octo::Channel::Adapters::Weixin::Adapter do
  let(:config) do
    {
      token:         "test-token",
  ...
  describe "#extract_files (private)" do
    ...
    it "extracts image items — downloads and returns data_url" do
      ...
      allow(adapter.instance_variable_get(:@api_client))
        ...
        .with(cdn_media, Octo::Channel::Adapters::Weixin::ApiClient::MEDIA_TYPE_IMAGE)
    ...
    it "prefers image_item.aeskey over media.aes_key for image download" do
      ...
      allow(adapter.instance_variable_get(:@api_client))
        ...
        .with({ "encrypt_query_param" => "abc", "aes_key" => top_aeskey },
              Octo::Channel::Adapters::Weixin::ApiClient::MEDIA_TYPE_IMAGE)
        .and_return(fake_jpeg)
  ...
  describe "#send_file" do
    ...
    it "returns message_id nil and logs error when no context_token exists" do
      expect(Octo::Logger).to receive(:warn).with(/no context_token/)
    ...
    it "catches and logs ApiClient errors" do
      ...
      allow(fake_client).to receive(:send_file).and_raise(
        Octo::Channel::Adapters::Weixin::ApiClient::ApiError.new(500, "CDN error")
      )

      expect(Octo::Logger).to receive(:error).with(/send_file failed/)
