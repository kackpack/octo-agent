...
require "spec_helper"
require "octo/server/channel/adapters/telegram/adapter"
require "octo/server/channel/adapters/telegram/api_client"

RSpec.describe Octo::Channel::Adapters::Telegram::Adapter do
  let(:config) do
    {
  ...
  before do
    ...
    adapter.instance_variable_set(:@bot_username, "octo_bot")
  end
  ...
  describe "#group_mention? (private)" do
    it "returns true when text contains @bot_username as a mention entity" do
      text = "hey @octo_bot do this"
      offset = text.index("@octo_bot")
      msg = { "entities" => [{ "type" => "mention", "offset" => offset, "length" => "@octo_bot".length }] }
    ...
    it "fails closed (returns false) when bot identity is unknown" do
      ...
      expect(adapter.send(:group_mention?, msg, "@octo_bot")).to eq(false)
    end
  end

  describe "#strip_bot_mention (private)" do
    it "removes the @bot_username token" do
      out = adapter.send(:strip_bot_mention, "@octo_bot please summarize")
    ...
    it "accepts group messages when @bot_username is mentioned, and strips the mention" do
      ...
      update = {
        ...
        "message"   => {
          ...
          "from"       => { "id" => 1002 },
          "text"       => text,
          "entities"   => [{ "type" => "mention", "offset" => offset, "length" => "@octo_bot".length }]
        }
      }
      adapter.process_update(update)
      expect(events.size).to eq(1)
