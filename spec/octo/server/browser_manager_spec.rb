...
require "stringio"
require "json"
require "octo/server/browser_manager"
require "octo/tools/browser"

RSpec.describe Octo::BrowserManager do
  # Each test gets a fresh instance — avoids singleton state leaking between examples.
  let(:manager) { described_class.new }

  # Temp dir to isolate browser.yml reads/writes from the real ~/.octo/browser.yml
  let(:tmp_dir)     { Dir.mktmpdir }
  let(:config_path) { File.join(tmp_dir, "browser.yml") }

  before do
    stub_const("Octo::BrowserManager::BROWSER_CONFIG_PATH", config_path)
    allow(Octo::Logger).to receive(:info)
    allow(Octo::Logger).to receive(:warn)
  end
  ...
  describe "#ensure_process! (private)" do
    ...
    it "starts a new daemon and completes the MCP handshake" do
      ...
      allow(Octo::Utils::BrowserDetector).to receive(:detect).and_return({
        status: :ok,
        mode: :ws_endpoint,
    ...
    it "raises BrowserNotReachableError when browser is not found" do
      allow(Octo::Utils::BrowserDetector).to receive(:detect).and_return({ status: :not_found })

      expect { manager.send(:ensure_process!) }.to raise_error(
        Octo::BrowserNotReachableError,
