...
# The server writes under ~/.octo/agents and ~/.octo/memories, so we
...
module HttpServerProfileSpecHelpers
  ...
  def build_server
    ...
    Octo::Server::HttpServer.new(
      ...
      sessions_dir:   Dir.mktmpdir("octo_profile_spec_sessions")
    )
  end
end

RSpec.describe Octo::Server::HttpServer, "profile + memories endpoints" do
  include HttpServerProfileSpecHelpers

  let!(:agents_dir) { Dir.mktmpdir("octo_agents_spec") }
  let!(:memories_dir) { Dir.mktmpdir("octo_memories_spec") }

  before do
    stub_const("Octo::Server::HttpServer::PROFILE_USER_AGENTS_DIR", agents_dir)
    stub_const("Octo::Server::HttpServer::MEMORIES_DIR", memories_dir)
  end
