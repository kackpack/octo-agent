...
require "tmpdir"
require "fileutils"
require "octo/server/scheduler"

RSpec.describe Octo::Server::Scheduler do
  let(:tmpdir) { Dir.mktmpdir("octo_scheduler_spec") }

  # Build a scheduler that uses tmpdir instead of ~/.octo
  subject(:scheduler) do
    ...
    stub_const("Octo::Server::Scheduler::SCHEDULES_FILE", File.join(tmpdir, "schedules.yml"))
    stub_const("Octo::Server::Scheduler::TASKS_DIR",      File.join(tmpdir, "tasks"))
    s
  end
