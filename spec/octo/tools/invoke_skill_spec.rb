...
RSpec.describe Octo::Tools::InvokeSkill do
  ...
  def build_agent(tmpdir)
    client = instance_double(Octo::Client).tap do |c|
      c.instance_variable_set(:@api_key, "test-api-key")
    end
    config = Octo::AgentConfig.new(model: "gpt-3.5-turbo", permission_mode: :auto_approve)
    agent  = Octo::Agent.new(client, config, working_dir: tmpdir, ui: nil,
                               profile: "general",
