...
RSpec.describe Octo::Utils::FileProcessor do
  # ---------------------------------------------------------------------------
  ...
  describe ".save" do
    ...
    it "preserves Chinese characters in filename" do
      result = described_class.save(body: "x", filename: "Octo企业智能体平台.pptx")
      expect(result[:name]).to eq("Octo企业智能体平台.pptx")
    end
    ...
    it "does NOT parse the file" do
      expect(Octo::Utils::ParserManager).not_to receive(:parse)
    ...
    context "when parser succeeds" do
      it "returns FileRef with preview_path written to disk" do
        Dir.mktmpdir do |dir|
          ...
          allow(Octo::Utils::ParserManager).to receive(:parse).with(path)
          ...
          expect(ref.preview_path).to start_with(Octo::Utils::FileProcessor::UPLOAD_DIR)
      ...
      it "uses filename as display name" do
        Dir.mktmpdir do |dir|
          ...
          allow(Octo::Utils::ParserManager).to receive(:parse)
        ...
        Dir.mktmpdir do |dir|
          ...
          allow(Octo::Utils::ParserManager).to receive(:parse).with(path)
            .and_return({ success: false, text: nil,
                          error: "pdftotext failed", parser_path: "/home/.octo/parsers/pdf_parser.rb" })

          ref = described_class.process_path(path)
          expect(ref.preview_path).to be_nil
          expect(ref.parse_error).to eq("pdftotext failed")
          expect(ref.parser_path).to eq("/home/.octo/parsers/pdf_parser.rb")
  ...
  describe ".process" do
    it "saves file to disk and returns parsed FileRef" do
      ...
      expect(ref).to be_a(Octo::Utils::FileProcessor::FileRef)
