...
RSpec.describe Octo::Utils::ScriptsManager do
  describe "DEFAULT_SCRIPTS_DIR" do
    it "points to the gem scripts/ directory (not a parent workspace)" do
      ...
      # The gem root is octo/ — confirm the resolved path contains it
      expect(dir).to include("octo")
  ...
  describe ".setup!" do
    ...
    before do
      stub_const("Octo::Utils::ScriptsManager::SCRIPTS_DIR", tmp_scripts_dir)
      stub_const("Octo::Utils::ScriptsManager::VERSION_FILE",
    ...
    it "writes a version stamp file after setup" do
      ...
      expect(File.read(version_file).strip).to eq(Octo::VERSION)
    end
    ...
    it "does not re-copy scripts when version is unchanged" do
      ...
      File.write(File.join(tmp_scripts_dir, ".version"), Octo::VERSION)
