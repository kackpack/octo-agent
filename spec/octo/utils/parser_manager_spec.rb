...
RSpec.describe Octo::Utils::ParserManager do
  # Use a temp dir as PARSERS_DIR so tests don't pollute ~/.octo/parsers/
  let(:tmp_parsers_dir) { Dir.mktmpdir }

  before do
    stub_const("Octo::Utils::ParserManager::PARSERS_DIR", tmp_parsers_dir)
  end
  ...
  describe ".setup!" do
    it "copies default parsers into PARSERS_DIR if not already present" do
      ...
      default_dir = Octo::Utils::ParserManager::DEFAULT_PARSERS_DIR
      skip "No default parsers found" unless Dir.exist?(default_dir) && !Dir.glob("#{default_dir}/*.rb").empty?

      described_class.setup!

      Octo::Utils::ParserManager::PARSER_FOR.values.uniq.each do |script|
    ...
    it "upgrades installed parser when bundled VERSION is newer, preserving old as .bak" do
      ...
      begin
        File.write(File.join(fake_defaults, "pdf_parser.rb"),
                   "# pdf_parser v99\n# VERSION: 99\nputs 'new'\n")
        stub_const("Octo::Utils::ParserManager::DEFAULT_PARSERS_DIR", fake_defaults)
