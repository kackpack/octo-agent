...
require "spec_helper"
require "octo/utils/arguments_parser"

RSpec.describe Octo::Utils::ArgumentsParser do
  let(:tool_registry) do
    registry = instance_double("Octo::ToolRegistry")
  ...
  describe ".parse_and_validate" do
    ...
    context "with XML contamination" do
      ...
      it "handles real-world example from error log" do
        ...
        call = {
          name: "file_reader",
          arguments: "{\"path\": \"lib/octo/ui2/components/modal_component.rb\", \"start_line\":315</parameter>\n<parameter name=\"end_line\": 330}"
        }
        
        result = described_class.parse_and_validate(call, tool_registry)
        
        expect(result[:path]).to eq("lib/octo/ui2/components/modal_component.rb")
        expect(result[:start_line]).to eq(315)
        expect(result[:end_line]).to eq(330)
      end
    ...
    context "with incomplete JSON" do
      ...
      it "handles literal backslash-n instead of real newline (session JSON format)" do
        ...
        call = {
          name: "file_reader",
          arguments: '{"end_line\":550</parameter>\n<parameter name=\"path":"octo/lib/octo/web/index.html","start_line":400}'
        }
        
        result = described_class.parse_and_validate(call, tool_registry)
        
        expect(result[:end_line]).to eq(550)
        expect(result[:path]).to eq("octo/lib/octo/web/index.html")
    ...
    context "with missing required parameters" do
      it "raises error for missing required params" do
        ...
        }.to raise_error(Octo::Utils::MissingRequiredParamsError, /Missing required parameters: path/)
      end
    end
