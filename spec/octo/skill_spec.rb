...
RSpec.describe Octo::Skill do
  ...
  describe "#process_content" do
    ...
    context "shell-style ${VAR} substitution from ENV" do
      it "substitutes known ENV variables" do
        ...
        File.write(File.join(skill_dir, "SKILL.md"), <<~CONTENT)
          ...
          ---
          Connect to http://${OCTO_SERVER_HOST}:${OCTO_SERVER_PORT}/api
        CONTENT

        orig_host = ENV["OCTO_SERVER_HOST"]
        orig_port = ENV["OCTO_SERVER_PORT"]
        ENV["OCTO_SERVER_HOST"] = "127.0.0.1"
        ENV["OCTO_SERVER_PORT"] = "8888"
        begin
          skill = described_class.new(skill_dir)
          result = skill.process_content
          expect(result).to include("http://127.0.0.1:8888/api")
          expect(result).not_to include("${OCTO_SERVER_HOST}")
          expect(result).not_to include("${OCTO_SERVER_PORT}")
        ensure
          ENV["OCTO_SERVER_HOST"] = orig_host
          ENV["OCTO_SERVER_PORT"] = orig_port
        end
      end
      ...
      it "substitutes ${VAR} even when template_context is empty" do
        ...
        File.write(File.join(skill_dir, "SKILL.md"), <<~CONTENT)
          ...
          description: Skill
          ---
          Port: ${OCTO_SERVER_PORT}
        CONTENT

        orig = ENV["OCTO_SERVER_PORT"]
        ENV["OCTO_SERVER_PORT"] = "9999"
        begin
          skill = described_class.new(skill_dir)
          result = skill.process_content(template_context: {})
          expect(result).to include("Port: 9999")
        ensure
          ENV["OCTO_SERVER_PORT"] = orig
        end
      end

      it "appends an environment hint when ${OCTO_SERVER_*} is referenced but not injected" do
        ...
        File.write(File.join(skill_dir, "SKILL.md"), <<~CONTENT)
          ...
          ---
          Call http://${OCTO_SERVER_HOST}:${OCTO_SERVER_PORT}/api/foo
        CONTENT

        orig_host = ENV.delete("OCTO_SERVER_HOST")
        orig_port = ENV.delete("OCTO_SERVER_PORT")
        begin
          skill = described_class.new(skill_dir)
          result = skill.process_content
          expect(result).to include("Environment note")
          expect(result).to include("octo server")
          # Unexpanded placeholders survive verbatim so the hint's context is clear.
          expect(result).to include("${OCTO_SERVER_HOST}")
        ensure
          ENV["OCTO_SERVER_HOST"] = orig_host if orig_host
          ENV["OCTO_SERVER_PORT"] = orig_port if orig_port
        end
      end

      it "does NOT append the environment hint when ${OCTO_SERVER_*} is fully expanded" do
    ...
    context "Supporting Files injection (plain skill)" do
      ...
      it "lists supporting files from skill directory" do
        dir = make_skill_with_scripts(temp_dir)
        skill = described_class.new(dir)
    ...
    context "encrypted skills removed in octo fork" do
      it "does not support encrypted skill files" do
        dir = File.join(temp_dir, "plain-skill")
        FileUtils.mkdir_p(dir)
        File.write(File.join(dir, "SKILL.md"), "---\nname: plain\ndescription: x\n---\ncontent")

        skill = Octo::Skill.new(dir)
        expect(skill).not_to respond_to(:encrypted?)
      end
    end
  end
