...
> **Pre-1.0。** 三种界面均已上线：CLI（终端里是交互式 TUI，其余场景是 headless 的 agentic 单发）、本地 Web 服务（`octo serve`）、IM 桥接（`octo channel`，微信 iLink）。在 agent 循环之上还有 skills、MCP 客户端、操作系统级沙箱、持久化记忆、子代理，以及用于自主多步目标的任务图。
...
| 记忆 | 完成 | `~/.octo/memory/` 下的跨会话持久化记忆，自动抽取/整合 |
| 子代理 | 完成 | `launch_agent` 并行扇出，异步 + 可恢复（`send_message`、`agent_status`、`kill_agent`） |
| 编排 | 完成 | `octo conduct` —— 把目标拆成子任务 DAG，经子代理执行，崩溃后可 resume |
| Web 服务 | 完成 | `octo serve` —— REST + SSE，内嵌仪表盘 UI（绑定 localhost） |
| IM 桥接 | 完成 | `octo channel` —— 微信 iLink 适配器（扫码登录、按用户隔离 session、斜杠命令） |
...
```
cmd/octo/          CLI 入口（chat 单发 + TUI / serve / channel / conduct / mcp / 斜杠命令）
   ↓
...
internal/conductor/  octo conduct —— 子任务 DAG 规划器 + 调度器
```
